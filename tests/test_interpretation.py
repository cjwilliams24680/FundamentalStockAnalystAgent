...
def test_raw_value_passes_through_unchanged():
    result = interpretation.interpret_current_ratio(1.75)
...
def test_building_block_sign_flags():
    assert interpretation.interpret_gross_profit(-50.0).falls_outside_normal_range is True
    assert interpretation.interpret_gross_profit(400.0).falls_outside_normal_range is False
    assert (
...
def test_effective_tax_rate_bands():
    assert interpretation.interpret_effective_tax_rate(0.01).falls_outside_normal_range is True
    assert interpretation.interpret_effective_tax_rate(0.21).falls_outside_normal_range is False
    assert interpretation.interpret_effective_tax_rate(0.45).falls_outside_normal_range is True
...
def test_solvency_and_leverage_bands():
    assert interpretation.interpret_debt_to_equity(0.5).falls_outside_normal_range is False
    assert interpretation.interpret_debt_to_equity(1.8).falls_outside_normal_range is True
    assert interpretation.interpret_debt_to_assets(0.3).falls_outside_normal_range is False
...
def test_valuation_bands():
    assert interpretation.interpret_price_to_earnings(8.0).falls_outside_normal_range is True
    assert interpretation.interpret_price_to_earnings(17.0).falls_outside_normal_range is False
    assert interpretation.interpret_price_to_earnings(60.0).falls_outside_normal_range is True
    assert interpretation.interpret_earnings_yield(-0.02).falls_outside_normal_range is True
    assert interpretation.interpret_earnings_yield(0.05).falls_outside_normal_range is False
    assert (
    ...
    assert interpretation.interpret_price_to_book(0.8).falls_outside_normal_range is True
...
def test_none_that_means_no_debt_is_good_news():
    result = interpretation.interpret_interest_coverage(None)
    assert "no debt" in result.interpretation
    assert result.falls_outside_normal_range is False
    result = interpretation.interpret_operating_cash_flow_to_debt(None)
    assert "debt-free" in result.interpretation
    assert result.falls_outside_normal_range is False


def test_none_that_points_at_sibling_metric():
    assert "earnings_yield" in interpretation.interpret_price_to_earnings(None).interpretation
    assert "debt_to_capital" in interpretation.interpret_debt_to_equity(None).interpretation
    assert (
        "sloan_accruals_ratio"
        in interpretation.interpret_operating_cash_flow_to_net_income(None).interpretation
    )
    assert (
...
def test_altman_score_bands():
    ...
    assert interpretation.interpret_altman_z_double_prime(3.0).falls_outside_normal_range is False


def test_altman_zone_labels():
    for interpret_zone in (
        interpretation.interpret_altman_z_zone,
        interpretation.interpret_altman_z_double_prime_zone,
    ):
        assert interpret_zone(metrics.Z_SAFE).falls_outside_normal_range is False
        assert interpret_zone(metrics.Z_GREY).falls_outside_normal_range is True
        assert interpret_zone(metrics.Z_DISTRESS).falls_outside_normal_range is True
        assert interpret_zone(None).falls_outside_normal_range is False


def test_altman_zone_rejects_unknown_label():
    with pytest.raises(ValueError):
        interpretation.interpret_altman_z_zone("purple")
    with pytest.raises(ValueError):
        interpretation.interpret_altman_z_double_prime_zone("purple")
